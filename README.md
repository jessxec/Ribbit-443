...
On the profile page, there is a section called Accuracy Tracker. This shows the user's accuracy for each tone based on the modules/lessons the user has completed. This feature is designed to provide users with insights into their performance and progress in mastering Chinese tones. However, the accuracy values are currently hard-coded, as we did not implement backend integration or dynamic data tracking due to time constraints.

Similarly, the streak values are hard-coded. The streak feature is theoretically meant to track the days a user is active/using the app. The gear icon on the profile header is just an image and doesn't navigate anywhere. However, this section is intended to house FAQs and settings in the future, but again, due to time constraints, this feature was not implemented.

The modules were designed to be completed in sequential order to ensure a structured learning experience, as each module unlocks with the user's collected stars from completing lessons. Initially, we planned to lock modules, allowing users to progress only after meeting the required stars. However, due to time constraints and the complexity of implementing the necessary features, all modules are currently accessible by default. The intended progression remains reflected in the module layout and content flow.

The badges unlocking feature is now functional, but the logic is hard-coded based on the current lesson. For example, specific badges unlock when completing designated lessons, such as "airport-navigator" unlocking after completing Lesson 3 in the Airport Module. Ideally, the badges would dynamically unlock based on the user's updated star count, providing a more robust and scalable approach. However, this feature was not implemented due to time constraints. With additional time and resources, integrating the star count into the badge-unlocking logic would ensure a more accurate and user-driven experience.
